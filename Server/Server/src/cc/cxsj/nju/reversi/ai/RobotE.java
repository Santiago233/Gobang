...
public class RobotE extends ReversiRobotAI{
		...
		for(int r=0 ; r<ROWS ; r++){
			for (int c=0 ; c<COLS ; c++){
				if(tboard[r][c].empty) {
					...
					score = evaluateColor(ownColor) - 9 * evaluateColor(oppositeColor) - Math.abs(r - 9) - Math.abs(c - 9);
					if(score > max_score){
						r1 = r;
						c1 = c;
						max_score = score;
	    ...
	    for(int r=0 ; r<ROWS ; r++){
			for (int c=0 ; c<COLS ; c++){
				if(tboard[r][c].empty) {
					tboard[r][c].color = ownColor;
					tboard[r][c].empty = false;
					score = evaluateColor(ownColor) - 101 * evaluateColor(oppositeColor) - Math.abs(r - 9) - Math.abs(c - 9);
					if(score > max_score){
						r2 = r;
						c2 = c;
						max_score = score;
